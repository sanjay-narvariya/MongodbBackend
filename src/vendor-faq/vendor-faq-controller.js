...
const VendorFaq = require("./vendor-faq-model")

exports.createFaq = catchAsyncErrors(async (req, res, next) => {
    try {
        const newFaq = await VendorFaq.create(req.body);
...
exports.getAllFaq = catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        const totalFaqs = await VendorFaq.countDocuments();

        const faq = await VendorFaq.find()
            .sort({ created_at: -1 })
...
exports.updateFaqByID = catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        const faq = await VendorFaq.findByIdAndUpdate(faqID, req.body, {
            new: true,
            runValidators: true,
        });
...
exports.deleteFaqByID = catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        const faqData = await VendorFaq.findById(faqID);

        if (!faqData) {
            return next(new ErrorHandler("FAQ Not Exist", 400));
        }

        const faq = await VendorFaq.deleteOne({ _id: faqID });
